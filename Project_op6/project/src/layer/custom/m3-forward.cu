...
#define ROW_TILE    2   // each thread computes 2 output rows
    ...
    int Height, int Width, int K)
{
    ...
    for (int t = 0; t < numTiles; ++t) {
        int k_col = t * TILE_WIDTH + tx;                // column index in A (mask)
        int k_row_base = t * TILE_WIDTH + ty * ROW_TILE; // row index in B (input), first of the two
