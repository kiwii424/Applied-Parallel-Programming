                                  ...
                                  int Batch, int Map_out, int Channel, int Height, int Width, int K) 
{
    ...
    const int KK        = K * K;
    const int H_unroll  = Channel * KK;                 
    const int W_unroll  = Batch * H_out * W_out;        

    const int numARows    = Map_out;                    
    const int numAColumns = H_unroll;                  
    const int numBRows    = H_unroll;                  
    const int numBColumns = W_unroll;                  
    ...
    #define MASK4D(m, c, p, q) mask[(m) * (Channel * K * K) + (c) * (K * K) + (p) * K + (q)]
    ...
    __shared__ float shC[WMMA_M * WMMA_N];    
