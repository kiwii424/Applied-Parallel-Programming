                                        ...
                                        const int Height, const int Width, const int K) 
{
    ...
    const size_t cols_per_img = H_out * W_out;  
    ...
    #define IN4D(i3, i2, i1, i0) input[(i3) * (Channel * Height * Width) + (i2) * (Height * Width) + (i1) * Width + (i0)]
    ...
    if (t < (size_t)Channel * cols_per_img) {
        ...
        const size_t col_id = b * cols_per_img + h_out * W_out + w_out; 

        const size_t KK = K * K;
        const size_t row_base = c * KK;  
...
__global__ void permute_output_kernel_op4(const float *input, float *output,
                                            ...
                                            const int Width, const int K)
{
    const size_t H_out = Height - K + 1;
    const size_t W_out = Width  - K + 1;
    const size_t H_unrolled = Channel * K * K;              
    const size_t W_unrolled = Batch * H_out * W_out;       
    ...
    // unrolling kernel
    {
        ...
        matrix_unrolling_kernel<<<grid_dim, block_dim>>>(device_input, unrolled_matrix, Batch,
    ...
    stat = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, (int)W_unrolled, Map_out, 
